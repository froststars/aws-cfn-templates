...
t.add_description('MySQL RDS instance.')
...
param_az = t.add_parameter(Parameter(
    'AvailabilityZone',
    Description='The name of the Availability Zone where the DB instance is '
                'located. This parameter has no effect when DatabaseMultiAZ '
...
param_db_engine_version = t.add_parameter(Parameter(
    ...
    Type='String',
    AllowedValues=cfnutil.load_mapping('mapping/rds-mysql-versions.json'),
))
...
t.add_condition(
    'MultiAzCondition',
    Equals(Ref(param_db_multi_az), 'true'),
)
...
rds_instance = t.add_resource(rds.DBInstance(
    ...
    # KmsKeyId=If('DefaultKmsCondition', Ref(AWS_NO_VALUE), Ref(param_db_kms_key)),
    ...
    AvailabilityZone=If('MultiAzCondition', AWS_NO_VALUE, Ref(param_az)),
